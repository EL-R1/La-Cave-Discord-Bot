...
module.exports = {
    ...
    async execute(client, message) {
        ...
        function setYarss(path_title, path_season, command) {
            ...
            const new_show_sub = JSON.parse(`{"active": true,"add_torrents_in_paused_state": "Default","auto_managed": "Default","custom_text_lines": "","download_location": "${process.env.DOWNLOAD_LOCATION}","email_notifications": {},"ignore_timestamp": false,"key": "0","label": "","last_match": "","max_connections": -2,"max_download_speed": -2,"max_upload_slots": -2,"max_upload_speed": -2,"move_completed": "move_completed","name": "One Piece","prioritize_first_last_pieces": "Default","regex_exclude": "(?i) FRENCH | MULTI |.mp4|AMZN|HULU|B-Global|S\d{2}E\d{2}-","regex_exclude_ignorecase": true,"regex_include": "(?i)One Piece.*1080p","regex_include_ignorecase": true,"rssfeed_key": "0","sequential_download": "Default"}`);
            ...
            const sortedWords = replaced_title.split(" ").sort((a, b) => b.length - a.length);
            const regex = sortedWords.split(" ").slice(0, 2).join(" ");
